(* ================================================================ *)
(*          Tic-Tac-Toe Step 3: Alpha-Beta Implementation           *)
(* ================================================================ *)
...
let evaluate_state (current_board : board) (ai_player_token : player) : int =
  ...
  else 999 (* Only called at terminal nodes *)

let rec alphabeta (current_board : board) (current_player_turn : player)
                   (ai_player_token : player) (alpha : int) (beta : int) : int =
  if check_win current_board ai_player_token then 10
  else if check_win current_board (opponent_of ai_player_token) then -10
  else if is_board_full current_board then 0
  else
    let possible_next_moves = available_moves current_board in
    if current_player_turn = ai_player_token then
      let best_score = ref (-1001) in
      List.iter (fun (r, c) ->
        match make_move current_board r c current_player_turn with
        | Some next_board ->
            let score = alphabeta next_board (opponent_of current_player_turn) ai_player_token alpha beta in
            best_score := max !best_score score;
            if beta <= !best_score then raise Exit
        | None -> ()
      ) possible_next_moves;
      !best_score
    else
...
let find_best_move_alphabeta (current_board : board) (ai_player_token : player) : (int * int) option =
  ...
  if List.length possible_moves = 0 then None
    ...
    List.iter (fun (r, c) ->
      ...
      | Some next_board ->
          let score = alphabeta next_board (opponent_of ai_player_token) ai_player_token (-1001) 1001 in
          if score > !best_score_ref then begin
...
let () =
  print_endline "\n--- Tic-Tac-Toe: Étape 3 Demo (Alpha-Beta) ---";

  let ai_player = O in
  let human_player = X in

  let board1 = [[Some O; Some O; None]; [Some X; Some X; None]; [None; None; None]] in
  print_endline "Board 1 (O to win):";
  display_board board1;
  match find_best_move_alphabeta board1 ai_player with
  | Some (r, c) -> Printf.printf "AI 'O' best move: (%d, %d) (Expected: (0,2))\n" r c
  | None -> print_endline "No move found.";

  let board2 = [[Some X; Some X; None]; [Some O; None; None]; [None; None; None]] in
  print_endline "\nBoard 2 (O to block X):";
  display_board board2;
  match find_best_move_alphabeta board2 ai_player with
  | Some (r, c) -> Printf.printf "AI 'O' best move: (%d, %d) (Expected: (0,2))\n" r c
  | None -> print_endline "No move found.";

  let board3 = initial_board () in
  let board3_after_x_corner = match make_move board3 0 0 human_player with Some b -> b | None -> board3 in
  print_endline "\nBoard 3 (X played corner, O's turn):";
  display_board board3_after_x_corner;
  match find_best_move_alphabeta board3_after_x_corner ai_player with
  | Some (r, c) -> Printf.printf "AI 'O' best move: (%d, %d) (Expected: center)\n" r c
  | None -> print_endline "No move found.";

  let board4 = [[Some X; None; Some O]; [None; Some X; None]; [Some O; None; None]] in
  print_endline "\nBoard 4 (O's turn):";
  display_board board4;
  match find_best_move_alphabeta board4 ai_player with
  | Some (r, c) -> Printf.printf "AI 'O' best move: (%d, %d) (Expected: (2,2))\n" r c
  | None -> print_endline "No move found.";

  let empty_board = initial_board () in
  print_endline "\nBoard 5 (Empty, AI 'X' first move):";
  display_board empty_board;
  match find_best_move_alphabeta empty_board X with
  | Some (r, c) -> Printf.printf "AI 'X' best move: (%d, %d) (Expected: corner or center)\n" r c
  | None -> print_endline "No move found.";
